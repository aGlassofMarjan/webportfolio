...
function initLoaderHome() {
  ...
  tl.call(function () {
    scroll.stop();
    scroll.scrollTo(targetAvailable, { immediate: true, force: true, lock: true, duration: 0.0166 });
  }, null, 0);
  ...
  function startHome() {
    scroll.scrollTo(targetAvailable, { immediate: true, force: true, lock: false, duration: 0.0166 });
    scroll.update();
...
function initPageTransitions() {
  ...
  function scrollToAvailable(container) {
    ...
    scroll.scrollTo(targetAvailableNext, { immediate: true, force: true, locked: true });
    scroll.update();
    initMimeScroll();
  }
  ...
  barba.init({
    ...
    transitions: [{
      ...
      async after(data) {
        if (data.trigger !== "back") { } else { window.scrollTo(0, scrollPosY) }
      }
    }, {
      name: 'default',
      once(data) {
      ...
      async after(data) {
        if (data.trigger !== "back") { window.scrollTo(0, 0); } else { window.scrollTo(0, scrollPosY); };
      }
    }]
  });
...
function initSplitText() {
  ...
  var splitChars = new SplitText(".split-chars", { type: "chars", charsClass: "single-char" });
